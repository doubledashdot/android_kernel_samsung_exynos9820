...
AK3_BRANCH="beyond0lte"
AK3_DIR="$(pwd)/android/AnyKernel3"

ZIPNAME="not-$(date '+%Y%m%d').zip"
TC_DIR="$(pwd)/tc/clang"
DEFCONFIG="exynos9820-beyond0lte_defconfig ksu.config"
...
if test -z "$(git rev-parse --show-cdup 2>/dev/null)" &&
   head=$(git rev-parse --verify HEAD 2>/dev/null); then
    ZIPNAME="${ZIPNAME::-4}-$(echo $head | cut -c1-8)-beyond0lte.zip"
fi
...
rm -rf AnyKernel3
echo "[*] Cloning AnyKernel3 for beyond0lte"
